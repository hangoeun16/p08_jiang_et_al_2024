...
def build_table_a1_assets_from_raw(
    ...
    ta_merge = total_assets_df[["bank_id", "bank_name", "size_category", "total_assets"]].copy()
    ...
    # Backfill "Total Asset" with authoritative total_assets from total_assets_df
    # for banks where the raw RCFD/RCON value is missing (e.g., due to inner joins
    # in build_table_a1_raw_frames dropping banks present in only one series).
    mask = bank_asset["Total Asset"].isna() | (bank_asset["Total Asset"] == 0)
    bank_asset.loc[mask, "Total Asset"] = bank_asset.loc[mask, "total_assets"]
    bank_asset = bank_asset.drop(columns=["total_assets"])
